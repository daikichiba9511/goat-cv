# GOAT - Design Document

## Overview

帳票・ドキュメント画像に対するComputer Visionアノテーションアプリケーション。
非同期共同編集・事前推論を最終目標とし、まずは同期的・単一ユーザーで小さく始める。

### Target Domain

- **対象画像**: 帳票、フォーム、請求書等のドキュメント画像
- **アノテーションタスク**:
  1. **Object Detection** — ドキュメント内のオブジェクト（テキストブロック、表、フィールド等）をBBox/Polygonで検出
...
- **BBox**: `{ x, y, width, height }` — 全て 0.0-1.0 の正規化座標
- **Polygon**: `{ points: [{x, y}, ...] }` — 全て 0.0-1.0 の正規化座標
