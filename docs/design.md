...
| Pre-Inference | Provider Adapter + persisted Candidate | [ADR-0006](adr/0006-pre-inference-boundary.md) |
...
- **Inference Run** — 1つのImageに対する外部モデル推論の実行、座標空間、model version、成功または失敗の記録
- **Candidate** — Inference Runが返したBBoxまたはPolygon候補。採用されるまでAnnotationとは別に保持する
...
Pre-Inference導入後はCandidate判断も同じTransactionへ含める。
採用するCandidateは同じRequest内の新規Annotation `client_id`を参照し、GraphとCandidateの片方だけが保存される状態を防ぐ。
詳細は[Pre-Inference Integration Specification](pre-inference.md#atomic-graph-save)を参照する。
