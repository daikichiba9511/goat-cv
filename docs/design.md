...
非同期共同編集と事前ラベル取り込みを最終目標とし、まずは同期的・単一ユーザーで小さく始める。
...
| Pre-Label Import | GOAT JSON + persisted PreLabel | [ADR-0007](adr/0007-pre-label-import-boundary.md) |
...
- **PreLabelImport** — 外部で生成済みの結果をProject単位で取り込んだ記録。Source情報と作成時刻を保持する
- **PreLabelImportImage** — Importに含まれたImageとcoordinate spaceの記録。PreLabelが0件でも保持する
- **PreLabel** — ImportしたBBoxまたはPolygon。採用されるまでAnnotationとは別に保持する
...
Pre-Label Import導入後はPreLabel判断も同じTransactionへ含める。
採用するPreLabelは同じRequest内の新規Annotation `client_id`を参照し、GraphとPreLabelの片方だけが保存される状態を防ぐ。
詳細は[Pre-Label Import Specification](pre-label-import.md#atomic-graph-save)を参照する。
...
| **5** | Pre-label import | 外部で生成済みの推論結果を事前ラベルとして取り込み、確認後にAnnotationへ変換 |
